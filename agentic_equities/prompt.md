...
8. DISCORD SUMMARY POST -- REQUIRED EVERY FIRING, not optional, not skippable just because steps 1-7 are done (added 2026-08-31, fixed 2026-08-31 same evening after the first two live firings under this spec produced zero actual Discord messages -- root cause found: the original version of this step told you to interpolate the composed message directly into a Python string inside a double-quoted shell `-c` argument, and virtually every real message contains a `$` dollar amount, which bash expands as a variable reference inside double quotes -- silently mangling or truncating the payload on every single real run. Fixed by removing ALL interpolation from the shell command entirely. Follow the two sub-steps below EXACTLY, in order, and do not shortcut by going back to inline string-building:
   ...
   - Total account value and its change vs the $300 starting basis, e.g. `Account: $299.12 (-0.29% vs $300 start)`.
