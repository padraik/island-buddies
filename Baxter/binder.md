...
*Last updated: August 10, 2026 (scanner-based sourcing ratified as the default screening method; positions.md correction-propagation rule added)*
...
**LVS (-$67 | Jul 29):** The Jul 20 standing order was to sell both contracts before the Jul 22 close. It never became a real order in the account -- nothing was placed, the position rode through its own earnings unprotected, and the stock printed a fresh 52-week low the next session. Closed nine days late for whatever was left. The lesson isn't the loss, it's the gap: a price-trigger ladder gets written as a resting GTC limit (fixed for LYFT, Jul 20), but a date-triggered "sell by Wednesday" order was never given the same treatment. **A standing order is not real until it exists as an actual order in the account, timed to fire. A sentence in a markdown file is not an order.**

**Ladder GTCs placed as day orders, not GTC (Aug 11, 2026, caught before it cost anything):** UAMY and JMIA's scale-out ladder triggers were placed the morning after entry, exactly per the Jul 20 rule, but both came back `time_in_force: gfd` when checked live -- good for the day, not good-till-canceled. The Jul 20 rule assumed the order rests until it fires or the position closes; a day order does neither, it silently disappears at the close if unfilled. Same failure shape as LVS, one layer deeper: that time the standing order was never placed at all, this time it was placed correctly and still wouldn't have survived past 4 PM. **Placing the order is necessary but not sufficient. Confirm `time_in_force` is GTC (or explicitly re-place the day order every session until it fires or the position closes) before treating a ladder trigger as actually armed.** Same night, same session: BTBT's ladder order was placed explicitly specifying GTC and came back `time_in_force: gtc`, confirmed live. The default without saying so is a day order; asking for GTC by name gets a real GTC. Simple fix, worth stating as the standing instruction going forward: **every ladder order gets placed as GTC explicitly, not left to the platform's default.**

**TRMB earnings date, wrong for six weeks (Jul 29, no loss, pure luck):** The ledger said Jul 30 from entry (Jun 16) through fifteen-plus session notes. The real date, per Trimble's own reporting cadence and two independent sources checked Jul 29, is early-to-mid August (Aug 5 or Aug 10, still not pinned to a press release). Nobody re-verified it in six weeks because the position never got close enough to a decision point to force the check -- the trim and the rally both happened to work anyway. **Getting away with a bad date is not the same as the date being right.** New standing practice: verify the earnings date at entry AND again inside the final 7 days before the recorded date, not only when a check-in happens to feel urgent.

**`fetch_option_history.py` returns corrupted data on some contracts (Aug 4, caught before it hit any ledger):** Pulling archived candles for DIS and UBER after their closes returned price series starting August 2025 -- over a year before either position existed -- with implausible values (DIS supposedly peaked at $11.63, a 1236% move live quotes minutes earlier directly contradicted). ABT and LYFT's existing archives are clean because they were built from real order-log fills (`get_option_orders` by chain_id), not this script's raw historicals output. Likely an instrument-matching bug on strike/expiry collisions. Until debugged, archive closed contracts from order logs and session-note marks, not this script.
...
**UAMY earnings date: right in the research, wrong in the ledger (-$34 | Aug 11, 2026, self-inflicted, caught by Michael's memory, not by process):** `research_UAMY.md`, written Aug 8, has the correct date in plain text: Q2 earnings Aug 11 PM, `get_earnings_results` verified true, sourced correctly per the Aug 7 standing rule. It even flagged the tight resulting buffer. The error was introduced logging the position into positions.md on Aug 10: written as "Aug 14, 2026 AM" against the research doc's own correct "Aug 11, 2026 PM," and repeated with full confidence through Monday's entry note, Monday's check-in, and Tuesday morning's check-in -- three separate write-ups, zero cross-checks against the source doc that had it right the whole time. Caught only because Michael questioned the date from memory around 2 PM on the actual day of the print, roughly 90 minutes before the close. This is a different failure shape than KR (wrong at the source, discovered and fixed in the research file, just never propagated) or TRMB (nobody re-checked for six weeks). Here the correct number existed in the repo the entire time, one file away, and nothing ever compared the two. **Standing rule: when a position is logged into positions.md, its catalyst date is copied character-for-character from the source research doc's `get_earnings_results` line, not retyped from memory or session context. Any time those two dates could plausibly differ, they need to be opened side by side before either the entry or the next check-in is written.**
...
**Puts Rule 2 amendment (Aug 7, 2026, revised same evening): no puts entry more than 21 days before the dated catalyst.** The original write-up overstated the mechanism -- it is not that long-dated options decay to worthless in general. Theta decay accelerates as expiration nears rather than running flat, which is exactly why far-dated positions (LEAPS and similar) are a standard, widely-used strategy elsewhere. Blanket "duration is the enemy" is wrong and was called out as such the same night it was written.

The real, narrower finding: for a deep-OTM, single-catalyst-dependent bet with no other expected source of favorable movement -- which is how this fund structures both its calls and its puts -- weeks of dead time before the one event that justifies the position burns extrinsic value with nothing offsetting it. On top of that, three of the back-test's four clean entries were also fighting momentum the whole wait (RCL rallied 26.5% into its own print rather than falling), which is a directional problem, not a time problem. The rule stands as a practical guard against that specific structure -- thin, far-OTM, one-event bets held through unproductive dead time -- not a claim about options duration in general. Not retroactive. Read together with the existing Rule 2 (confirmed catalyst, min 21 DTE at entry): entry should land close to the catalyst with enough DTE left to trade it, not far ahead of it with nothing to do but wait.
...
**Earnings dates come from `get_earnings_results` first, WebSearch second (Aug 7, 2026, written after KR entered real money on a wrong date).** KR's $61C Sep 4 2026 was bought believing Q2 earnings fell on Sep 4 -- confirmed by WebSearch three separate times (Aug 4, Aug 6, Aug 7) before entry. The real date was Sep 10. All three "confirmations" were aggregator sites echoing the same stale prior-year data point, not independent verification -- multiple search hits agreeing is not the same as multiple sources agreeing. `get_earnings_results` returns the company's trailing 8 quarters plus the upcoming date with a `verified` flag (true = company-announced, false = estimated from cadence) in one free structured call, and would have shown Sep 4 didn't fit Kroger's own pattern (Sep 11 2025, Jun 20 2025, Mar 6 2025) on sight. **Standing rule: pull `get_earnings_results` before trusting any catalyst date, for screening (free, replaces a paid search on every calendar-trap kill) and especially before live entry. If it returns `verified: false` on a position going in with real money, that's a reason for one confirming WebSearch, not a reason to skip the tool.**

**Scanner-based sourcing is the default candidate-generation method, ratified Aug 10, 2026 (method run and cross-checked Aug 7, 2026).** Manual sector-by-sector ticker picking is superseded as the default -- it stays available as a fallback (scanner unavailable, or checking one specific name outside its regular pass) but is not what a screening session opens with anymore. Standard method: run the live market scanner (`get_scanner_filter_specs` for valid filter types, `create_scan` / `update_scan_filters`, `run_scan`) across market-cap bands spanning the full tradeable universe (roughly $300M-$150B+, split across ~7 queries -- no single query covers the whole range), compute each name's 52-week range percentile by hand from the returned high/low/price columns (there is no direct percentile filter as of Aug 7; three filter-value formats were tried and failed, worth re-testing if the scanner's spec list changes), and keep bottom-quartile names as the CALLS-side raw pool (top-quartile for PUTS) -- this is Step 1 of THE UNIFIED SCREEN, now computed programmatically instead of guessed at by sector. Narrow that pool with `get_earnings_results` to names reporting inside the next 35 days -- the actual tradeable window -- before spending a single chain check or search. The comparison that ratified this: the same night, the old method (510 names hand-picked and researched) found zero survivors, while the scanner-plus-earnings-tool method covered the entire market (202 bottom-quartile names) and produced a real 48-name queue, for free. Not a bigger-edge claim -- a cheaper, more complete process, and the result speaks for itself. Full mechanics and the Aug 7 comparison: `Correspondence/2026-08-07_from_Baxter_screening_upgrade.md`.

**A correction is not complete until positions.md reflects it, not just the file where it was found (Aug 10, 2026, written after KR).** KR's real catalyst date (Sep 10, not Sep 4) was discovered, verified, and logged into `research_KR.md` and correspondence on Aug 7 -- the same night it happened. It never made it into positions.md's Open Positions table, the one file read at the start of every session and the one both Baxter and Michael treat as "what's actually true right now." The stale Sep 4 date sat there three days until an unrelated order-log pull on Aug 10 caught it -- by which point the position should already have been closed. **Standing rule: any correction to a fact that also appears in positions.md's Fund State or Open Positions tables gets a same-session edit to positions.md itself, not just the file where the error was originally found.** This is now the binding reading of the Intra-session data correction protocol below whenever the two overlap.
...
**`fetch_puts_chain.py` STRIKE column rounded to the nearest whole dollar, fixed Aug 8, 2026.** The displayed strike used `.0f` formatting while ask/breakeven/needs% were computed correctly against the real fractional strike, so a $1.50 and a $2.00 contract could both print as "$2" on the same line's neighbors, hiding which one a given row actually was. Found while chain-checking the scanner's 48-name candidate list: 16 of 41 survivors had a real half-dollar strike, 6 of those had no real contract at all at the rounded integer, and PLUG's "$2 strike, needs 0.1%" line turned out to be the real $1.50 strike priced at exact intrinsic value with zero time premium, six days to expiry, not the healthier $2.00 contract sitting one row below it. Fixed to `.2f`, verified against PLUG's real chain post-fix. Script lives outside the repo (`C:\lifecoach\scripts\`, not under git) so this note is the only record of the change.

**The Five-Baxter debate lives inside `research_TICKER.md`, not a separate story doc (clarified Aug 4, 2026 after a real miss on KR).** The template (`research_template.md`) builds Bullxter/Bearxter/Calxter/Macxter "takes" plus the meeting transcript directly into the research file -- that's the default for any name reaching a scoreable conviction (3.5+). Separate `week-XX/story/five_baxters_*.md` files are reserved for structurally significant moments only (SOUN was the first survivor of the entire 202-name run -- a landmark, not routine). Names that die in DD before reaching a scoreable conviction (BTG, MOS) don't get a debate section at all -- there's nothing to argue about a name Rule 6 already killed. Skipping the debate entirely, or bolting it on as an afterthought file once conviction is claimed, is the failure mode this note exists to prevent.
